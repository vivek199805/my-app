...
@Component({
  selector: 'app-create-aadharpey',
  templateUrl: './create-aadharpey.component.html',
  styleUrls: ['./create-aadharpey.component.css']
})
export class CreateAadharpeyComponent implements OnInit {
      ...
      this._auth.postdata(formdata, config.template.create).subscribe((res: any) => {
        if (res.statuscode == 200) {
          Swal.fire({
            ...
            this.router.navigate(['/commission/list-template/list-aadharpay'])
          });
        } else {
          Swal.fire({
            icon: 'error',
