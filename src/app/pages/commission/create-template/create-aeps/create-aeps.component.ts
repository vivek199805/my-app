...
@Component({
  selector: 'app-create-aeps',
  templateUrl: './create-aeps.component.html',
  styleUrls: ['./create-aeps.component.css']
})
export class CreateAepsComponent implements OnInit {
    ...
    this.auth.postdata(formdata, config.template.create).subscribe((res: any) => {
      if (res.statuscode == 200) {
        Swal.fire({
          ...
          this.router.navigate(['/commission/list-template/aeps-list'])
        });
      } else {
        Swal.fire({
          icon: 'error',
