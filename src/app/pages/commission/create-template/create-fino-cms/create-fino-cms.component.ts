...
@Component({
  selector: 'app-create-fino-cms',
  templateUrl: './create-fino-cms.component.html',
  styleUrls: ['./create-fino-cms.component.css']
})
export class CreateFinoCmsComponent implements OnInit {
      ...
      this._auth.postdata(formdata, config.template.create).subscribe((res: any) => {
        if (res.statuscode == 200) {
          Swal.fire({
            ...
            this.router.navigate(['/commission/list-template/list-fino-cms'])
          });
        } else {
          Swal.fire({
            icon: 'error',
