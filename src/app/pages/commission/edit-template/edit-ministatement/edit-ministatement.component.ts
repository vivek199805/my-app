...
@Component({
  selector: 'app-edit-ministatement',
  templateUrl: './edit-ministatement.component.html',
  styleUrls: ['./edit-ministatement.component.css']
})
export class EditMinistatementComponent implements OnInit {
    ...
    this._auth.postdata(formdata, config.template.update).subscribe((res: any) => {
      if (res.statuscode == 200) {
        Swal.fire({
          ...
          this.router.navigate(['/commission/list-template/list-ministatement'])
        });
      } else {
        Swal.fire({
          icon: 'error',
