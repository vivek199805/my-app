...
@Component({
  selector: 'app-edit-mobikwik-wallet',
  templateUrl: './edit-mobikwik-wallet.component.html',
  styleUrls: ['./edit-mobikwik-wallet.component.css']
})
export class EditMobikwikWalletComponent implements OnInit {
    ...
    this._auth.postdata(formdata, config.template.update).subscribe((res: any) => {
      if (res.statuscode == 200) {
        Swal.fire({
          ...
          this.router.navigate(['/commission/list-template/list-mobikwik'])
        });
      } else {
        Swal.fire({
          icon: 'error',
