...
@Component({
  selector: 'app-edit-lic',
  templateUrl: './edit-lic.component.html',
  styleUrls: ['./edit-lic.component.css']
})
export class EditLicComponent implements OnInit {
  DMTForm: FormGroup;
    ...
    this.auth.postdata(formdata, config.template.update).subscribe((res: any) => {
      if (res.statuscode == 200) {
        Swal.fire({
          ...
          this.router.navigate(['/commission/list-template/dmt-list'])
        });;
      } else {
        Swal.fire({
          icon: 'error',
