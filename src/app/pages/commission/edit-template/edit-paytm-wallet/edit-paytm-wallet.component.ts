...
@Component({
  selector: 'app-edit-paytm-wallet',
  templateUrl: './edit-paytm-wallet.component.html',
  styleUrls: ['./edit-paytm-wallet.component.css']
})
export class EditPaytmWalletComponent implements OnInit {
    ...
    this._auth.postdata(formdata, config.template.update).subscribe((res: any) => {
      if (res.statuscode == 200) {
        Swal.fire({
          ...
          this.router.navigate(['/commission/list-template/list-paytm'])
        });
      } else {
        Swal.fire({
          icon: 'error',
