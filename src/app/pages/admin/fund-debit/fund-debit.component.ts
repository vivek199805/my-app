...
@Component({
  selector: 'app-fund-debit',
  templateUrl: './fund-debit.component.html',
  styleUrls: ['./fund-debit.component.css']
})
export class FundDebitComponent implements OnInit {
  ...
  getServerResponse(val: any) {
    if (val && val != this.prevVal) {
      ...
      this.auth.postdata(formdata, config.userTypeList1).subscribe((res: any) => {        
  ...
  debit(){
    ...
    if (!this.debitbalance.valid) {
      ...
      this.auth.postdata(formdata, config.opration.funddebit).subscribe((res: any) => {
        if (res.statuscode == 200) {
          Swal.fire({
            title: res.message,
            icon: 'success'
